[Personal Learning Record](../../personal_learning_record/personal_learning_record.md) | [IT Proposals](../proposals/README.md) 

# Council Proposal 1

## Explanation of this proposal
*What do you think this proposal might mean? How might it be implemented?*

Informed by the session on Azure cloud computing, I propose gradually migrating the council’s on-premises server software into cloud-based services. Cloud platforms offer scalability, improved resilience, automated backups, and reduced physical infrastructure costs. By leveraging virtual machines and managed services in the cloud, applications can be modernised without sacrificing performance or security. The hands on work I completed with virtual machines in Session 10 has given me a practical insight into securing, and maintaining cloud environments, making this proposal both feasible and technically justified.

## Technologies and research relevant to this proposal
*Which class sessions and personal research refers to technology in this proposal. Link to examples.*

Comparing IaaS, PaaS, and SaaS models

Cloud migration strategies (Lift-and-Shift, Refactor, Rebuild)

Public-sector compliance requirements (GDPR, UK Cloud Governance Models)

Session 10 (Azure Cloud Computing): Provided hands-on experience deploying virtual machines and understanding cloud storage, scalability, and resilience

## Initial Analysis of the Proposal
*Use some simple analysis tools to help you think through whether the proposal is a good idea*



### SWOT Analysis
*You can use html tables in markdown*
 <table>
  <tr>
    <th>Strength</th>
    <th>Weakness</th>
  </tr>
  <tr>
    <td>
      <ol>
        <li>Increased scalability and availability</li>
        <li>improved performance through distributed resources</li>
        <li>Reduced dependency on physical infrastructure</li>
      </ol> 
    </td>
    <td>
      <ol>
        <li>Requires skilled staff for cloud administration</li>
        <li>Ongoing cloud subscription cost</li>
        <li>Potential vendor lock-in</li>
      </ol> 
    </td>
  </tr>
  <tr>
    <th>Opportunity</th>
    <th>Threat</th>
  </tr>
  <tr>
    <td>
      <ol>
        <li>Ability to modernise legacy applications</li>
        <li>Access to AI, automation, and analytics tools</li>
        <li>Supports digital-transformation strategies</li>
      </ol> 
    </td>
    <td>
      <ol>
        <li>Cloud outages could affect essential services</li>
        <li>Regulatory challenges regarding data sovereignty</li>
        <li>Misconfiguration risks could lead to data breaches</li>
      </ol> 
    </td>
  </tr>
</table> 

### PEST Analysis
*You can use html tables in markdown*

 <table>
  <tr>
    <th>Political</th>
    <th>Economic</th>
  </tr>
  <tr>
    <td>
      <ol>
        <li>Need to comply with GDPR and data residency regulations</li>
        <li>governance obligations for cloud-stored data</li>
        <li>Alignment with UK digital transformation and cloud-first strategies</li>
      </ol> 
    </td>
    <td>
      <ol>
        <li>Reduced on site hardware maintenance costs</li>
        <li>Subscription based cloud services impact long term budgets</li>
        <li>calability reduces cost during low demand periods</li>
      </ol> 
    </td>
  </tr>
  <tr>
    <th>Social</th>
    <th>Technological</th>
  </tr>
  <tr>
    <td>
      <ol>
        <li>Public expectation for faster and more reliable online services</li>
        <li>Staff require upskilling in cloud technologies</li>
        <li>Improved service availability benefits citizen satisfaction</li>
      </ol> 
    </td>
    <td>
      <ol>
        <li>Cloud offers stronger redundancy and failover capabilities</li>
        <li>Legacy systems may require ectra work to migrate</li>
        <li>Integration with modern SaaS applications becomes easier</li>
      </ol> 
    </td>
  </tr>
</table> 

## Questions to ask about the proposal
*Without knowing how exactly the council will implement this proposal, what questions should be raised?*

* question 1

*If the council is to use a cloud provider what provider would they use? 

* question 2

* How much does the council plan to spend and is it affordable?

## Evaluation of the proposal
*Povide a breif evaluation based on what you know*

The cloud computing knowledge I gained in Session 10 makes this proposal highly viable. Moving server workloads into the cloud would increase scalability and resilience. However, migration planning is crucial, especially for legacy software that may require refactoring. While cloud services introduce ongoing costs, they reduce maintenance overhead and improve service availability. My understanding of virtual machines, resource provisioning, and system administration supports the feasibility of this proposal, provided security and compliance considerations are carefully managed
